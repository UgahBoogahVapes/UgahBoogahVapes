        ...
        <div class="col-12">
            <div class="card mb-4">
                ...
                <ul class="nav nav-tabs mx-4 my-1">
                    <li class="nav-item">
                        <a class="tab nav-link <?php echo !isset($_GET['tab']) ? "active" : "" ?>" href="?page=manage">Active</a>
                    </li>
                    <li class="nav-item">
                        <a class="tab nav-link <?php echo isset($_GET['tab']) ? "active" : "" ?>" href="?page=manage&tab=archived">Archived</a>
                    </li>
                </ul>
...
<?php
    include './components/modals/add_user.php';
    include './components/modals/edit_user.php';
    include './components/modals/archive_user.php';
    include './components/modals/restore_user.php';
    $records = $crud->search("users", !isset($_GET['tab']) ? 0 : 1, ["archived"]);
    if ($records) {
...
<script>
    let gridApi;
    const rowData = <?php echo json_encode($crud->search("users", !isset($_GET['tab']) ? 0 : 1, ["archived"])); ?>;
    // Ito nmn dito me naglagay ng edit and delete button
    ...
    const gridOptions = {
        ...
        columnDefs: [
            ...
            { field: "full_name", flex: 1, headerName: "Full Name"  },
            { field: "username", flex: 1, headerName: "Username"  },
            { field: "id", headerName: "Actions", flex: 1, cellRenderer: (params) => actionButtons(params.value) }
        ],
        loading: false,
    };

<div class="modal fade" id="orderModal" tabindex="-1" role="dialog" aria-labelledby="orderModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 40em !important;" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Order Product</h5>
                <button type="button" onclick="map.invalidateSize();"
                    style="all: unset; cursor: pointer; width: 0.8em; height: 0.8em;" class="me-3" class="close"
                    data-dismiss="modal" aria-label="Close">
                    <svg style="width: 0.8em; height: 0.8em;" viewBox="0 0 24 24" version="1.1"
                        ...
                        <g id="SVGRepo_icurrentColoronCarrier">
                            <title>currentColorancurrentColorel</title>
                            <descurrentc>Created with SketcurrentColorh.</descurrentc>
                            <g id="icurrentColorons" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <g id="ui-gambling-website-lined-icurrentColornos-currentColorasinoshunter"
                                    transform="translate(-2168.000000, -158.000000)" fill="currentColor"
                                    fill-rule="nonzero">
                                    <g id="1" transform="translate(1350.000000, 120.000000)">
                                        <path
                                            ...
                                            id="currentColorancurrentColorel"> </path>
                                    </g>
                                </g>
                            </g>
                        </g>
            ...
            <?php
                $product_order = $crud->read_all("product_order");
                $order_pending = false;
                if ($product_order) {
                    if ($product_order[0]['status'] == "pending") {
                        $order_pending = true;
                    }
                }
            ?>
            <form action="./modules/<?php echo $order_pending ? "confirm_product" : "order_product" ?>.php" method="post" enctype="multipart/form-data">
                <input type="hidden" name="user" value="<?php echo $user['full_name'] ?>">
                <div id="info">
                    <div class="card">
                        <div class="card-body">
                            <div id="order-container">
                                <?php
                                    if ($product_order) {
                                        if ($product_order[0]['status'] == "pending") {
                                            $parsedOrder = json_decode($product_order[0]['products'], true);
                                            echo '
                                                <input type="hidden" name="order_id" value="'.$product_order[0]['id'].'" />
                                            ';
                                            foreach($parsedOrder as $order) {
                                                echo '
                                                    <div class="row">
                                                        <div class="col-md-8">
                            ...
                            <div style="display: <?php echo !$order_pending ? "flex" : "none" ?>; justify-content: center; align-items: center;">
                                <div class="dropup">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-center" aria-labelledby="dropdownMenuButton">
                                        <?php
                                            $products = $crud->read_all("inventory");
                                            if ($products) {
                                                foreach($products as $product) {
                                                    echo '<button type="button" id="product'.$product['id'].'" class="dropdown-item" onclick="createOrder('.$product['id'].', `'.$product['product_name'].'`)">'.$product['product_name'].'</button>';
                                                }
                                            }
                                        ?>
                                    </div>
                    ...
                    <div style="display: flex; justify-content: end; align-items: center; padding: 0 1em;">
                        <div style="display: flex; justify-content: end; gap: 0.6em;">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-info <?php echo $order_pending ? "d-none" : "" ?>" id="add-order-btn" disabled>Order</button>
                            <button type="submit" class="btn btn-success <?php echo !$order_pending ? "d-none" : "" ?>" id="confirm-order-btn">Confirm</button>
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    const createOrder = (id, name) => {
        ...
        document.getElementById("add-order-btn").disabled = false;
        ...
        // Append hidden input to the row
        rowDiv.appendChild(hiddenNameInput);
        rowDiv.appendChild(hiddenInput);

        // Create the first column div
        let colDiv1 = document.createElement('div');
        colDiv1.classList.add('col-md-7');

        // Create the form group div and product label
        let formGroup1 = document.createElement('div');
        formGroup1.classList.add('form-group');

        let labelProduct = document.createElement('label');
        labelProduct.setAttribute('for', 'product');
        labelProduct.classList.add('form-control-label');
        labelProduct.textContent = 'Product';

        let productDiv = document.createElement('div');
        productDiv.classList.add('form-control');
        productDiv.textContent = name;

        // Append elements to the form group and column
        formGroup1.appendChild(labelProduct);
        formGroup1.appendChild(productDiv);
        ...
        labelQuantity.setAttribute('for', 'example-text-input');
        labelQuantity.classList.add('form-control-label');
        ...
        // Append elements to the form group and column
        formGroup2.appendChild(labelQuantity);
        formGroup2.appendChild(quantityInput);
        colDiv2.appendChild(formGroup2);
        rowDiv.appendChild(colDiv2);

        // Create the third column div with delete button
        let colDiv3 = document.createElement('div');
        colDiv3.classList.add('col-md-1', 'd-flex');
        colDiv3.style.flexDirection = 'column';
        colDiv3.style.justifyContent = 'center';
        ...
        path1.setAttribute('d', 'M10 11V17');
        ...
        path2.setAttribute('d', 'M14 11V17');
        ...
        path4.setAttribute('d', 'M6 7H12H18V18C18 19.6569 16.6569 21 15 21H9C7.34315 21 6 19.6569 6 18V7Z');
        ...
        path5.setAttribute('d', 'M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5V7H9V5Z');
        ...
        deleteIcon.appendChild(path4);
        deleteIcon.appendChild(path5);

        // Append the SVG to the button and button to the column
        deleteButton.appendChild(deleteIcon);
        colDiv3.appendChild(deleteButton);
        rowDiv.appendChild(colDiv3);

        // Append the entire row to the desired parent element in your DOM
        document.getElementById("order-container").appendChild(rowDiv);
        console.log(document.getElementById("order-container"));
        setTimeout(() => {
            console.log(document.getElementById("order-container"));
